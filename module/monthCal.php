...
class LunarDate {
public $day, $month, $year, $leap, $jd;
public function __construct($dd, $mm, $yy, $leap, $jd) {
	$this->day = $dd;
	$this->month = $mm;
	$this->year = $yy;
	$this->leap = $leap;
...
function decodeLunarYear($yy, $k) {
	...
	if ($leapMonth == 0) {
		for($mm = 1; $mm <= 12; $mm++) {
			array_push($ly, new LunarDate(1, $mm, $yy, 0, $currentJD));
		...
		array_push($ly,new LunarDate(1, $leapMonth, $yy, 1, $currentJD));
...
function jdn2day($jd) {
	...
	for($row = 0; $row < 6; $row++){
		...
		for($col = 0; $col < 7; $col ++){
			...
			$calTable .= "<div id='cell".$index."' class='".$colClass."' onclick='selectDay(".$index.");'><div class='duong".$disableClass."' id='duong".$index."'>".$duong."</div><div class='am' id='am".$index."'>".$lDay."</div></div>";
		}
		$calTable .= "</div>";
	}
	echo $calTable;
